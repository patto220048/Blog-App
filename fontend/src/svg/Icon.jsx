export const darkIcon = (
    <svg width="22" height="22" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            d="M16 25C16.5128 25 16.9355 25.386 16.9933 25.8834L17 26V29C17 29.5523 16.5523 30 16 30C15.4872 30 15.0645 29.614 15.0067 29.1166L15 29V26C15 25.4477 15.4477 25 16 25Z"
            fill="url(#paint0_linear_402_4102)"
        />
        <path
            d="M22.0712 22.0711C22.4317 21.7106 22.9989 21.6829 23.3912 21.9879L23.4854 22.0711L25.6067 24.1924C25.9973 24.5829 25.9973 25.2161 25.6067 25.6066C25.2463 25.9671 24.679 25.9948 24.2867 25.6898L24.1925 25.6066L22.0712 23.4853C21.6807 23.0948 21.6807 22.4616 22.0712 22.0711Z"
            fill="url(#paint1_linear_402_4102)"
        />
        <path
            d="M8.5148 22.0711C8.90532 21.6806 9.53849 21.6806 9.92901 22.0711C10.2895 22.4316 10.3172 22.9988 10.0122 23.3911L9.92901 23.4853L7.80769 25.6066C7.41717 25.9971 6.784 25.9971 6.39348 25.6066C6.033 25.2461 6.00527 24.6789 6.31029 24.2866L6.39348 24.1924L8.5148 22.0711Z"
            fill="url(#paint2_linear_402_4102)"
        />
        <path
            d="M29 15C29.5523 15 30 15.4477 30 16C30 16.5128 29.614 16.9355 29.1166 16.9933L29 17H26C25.4477 17 25 16.5523 25 16C25 15.4872 25.386 15.0645 25.8834 15.0067L26 15H29Z"
            fill="url(#paint3_linear_402_4102)"
        />
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M10 16C10 12.6863 12.6863 10 16 10C19.3137 10 22 12.6863 22 16C22 19.3137 19.3137 22 16 22C12.6863 22 10 19.3137 10 16ZM20 16C20 13.7909 18.2091 12 16 12C13.7909 12 12 13.7909 12 16C12 18.2091 13.7909 20 16 20C18.2091 20 20 18.2091 20 16Z"
            fill="url(#paint4_linear_402_4102)"
        />
        <path
            d="M6 15C6.55228 15 7 15.4477 7 16C7 16.5128 6.61396 16.9355 6.11662 16.9933L6 17H3C2.44772 17 2 16.5523 2 16C2 15.4872 2.38604 15.0645 2.88338 15.0067L3 15H6Z"
            fill="url(#paint5_linear_402_4102)"
        />
        <path
            d="M24.4855 6.10051C24.876 5.70999 25.5092 5.70999 25.8997 6.10051C26.2602 6.46099 26.2879 7.02823 25.9829 7.42052L25.8997 7.51472L23.7784 9.63604C23.3879 10.0266 22.7547 10.0266 22.3642 9.63604C22.0037 9.27556 21.976 8.70833 22.281 8.31604L22.3642 8.22183L24.4855 6.10051Z"
            fill="url(#paint6_linear_402_4102)"
        />
        <path
            d="M6.10051 6.10051C6.46099 5.74003 7.02823 5.7123 7.42052 6.01732L7.51472 6.10051L9.63604 8.22183C10.0266 8.61236 10.0266 9.24552 9.63604 9.63604C9.27556 9.99653 8.70833 10.0243 8.31604 9.71923L8.22183 9.63604L6.10051 7.51472C5.70999 7.1242 5.70999 6.49103 6.10051 6.10051Z"
            fill="url(#paint7_linear_402_4102)"
        />
        <path
            d="M16 2C16.5128 2 16.9355 2.38604 16.9933 2.88338L17 3V6C17 6.55228 16.5523 7 16 7C15.4872 7 15.0645 6.61396 15.0067 6.11662L15 6V3C15 2.44772 15.4477 2 16 2Z"
            fill="url(#paint8_linear_402_4102)"
        />
        <defs>
            <linearGradient id="paint0_linear_402_4102" x1="16" y1="25" x2="16" y2="30" gradientUnits="userSpaceOnUse">
                <stop stopColor="#D771F1" />
                <stop offset="0.494792" stopColor="#F3E5F4" stopOpacity="0.95" />
                <stop offset="1" stopColor="#E6E7FD" stopOpacity="0.86" />
            </linearGradient>
            <linearGradient
                id="paint1_linear_402_4102"
                x1="23.839"
                y1="21.7782"
                x2="23.839"
                y2="25.8995"
                gradientUnits="userSpaceOnUse"
            >
                <stop stopColor="#D771F1" />
                <stop offset="0.494792" stopColor="#F3E5F4" stopOpacity="0.95" />
                <stop offset="1" stopColor="#E6E7FD" stopOpacity="0.86" />
            </linearGradient>
            <linearGradient
                id="paint2_linear_402_4102"
                x1="8.16125"
                y1="21.7782"
                x2="8.16125"
                y2="25.8995"
                gradientUnits="userSpaceOnUse"
            >
                <stop stopColor="#D771F1" />
                <stop offset="0.494792" stopColor="#F3E5F4" stopOpacity="0.95" />
                <stop offset="1" stopColor="#E6E7FD" stopOpacity="0.86" />
            </linearGradient>
            <linearGradient
                id="paint3_linear_402_4102"
                x1="27.5"
                y1="15"
                x2="27.5"
                y2="17"
                gradientUnits="userSpaceOnUse"
            >
                <stop stopColor="#D771F1" />
                <stop offset="0.494792" stopColor="#F3E5F4" stopOpacity="0.95" />
                <stop offset="1" stopColor="#E6E7FD" stopOpacity="0.86" />
            </linearGradient>
            <linearGradient id="paint4_linear_402_4102" x1="16" y1="10" x2="16" y2="22" gradientUnits="userSpaceOnUse">
                <stop stopColor="#D771F1" />
                <stop offset="0.494792" stopColor="#F3E5F4" stopOpacity="0.95" />
                <stop offset="1" stopColor="#E6E7FD" stopOpacity="0.86" />
            </linearGradient>
            <linearGradient
                id="paint5_linear_402_4102"
                x1="4.5"
                y1="15"
                x2="4.5"
                y2="17"
                gradientUnits="userSpaceOnUse"
            >
                <stop stopColor="#D771F1" />
                <stop offset="0.494792" stopColor="#F3E5F4" stopOpacity="0.95" />
                <stop offset="1" stopColor="#E6E7FD" stopOpacity="0.86" />
            </linearGradient>
            <linearGradient
                id="paint6_linear_402_4102"
                x1="24.1319"
                y1="5.80762"
                x2="24.1319"
                y2="9.92894"
                gradientUnits="userSpaceOnUse"
            >
                <stop stopColor="#D771F1" />
                <stop offset="0.494792" stopColor="#F3E5F4" stopOpacity="0.95" />
                <stop offset="1" stopColor="#E6E7FD" stopOpacity="0.86" />
            </linearGradient>
            <linearGradient
                id="paint7_linear_402_4102"
                x1="7.86828"
                y1="5.80762"
                x2="7.86828"
                y2="9.92894"
                gradientUnits="userSpaceOnUse"
            >
                <stop stopColor="#D771F1" />
                <stop offset="0.494792" stopColor="#F3E5F4" stopOpacity="0.95" />
                <stop offset="1" stopColor="#E6E7FD" stopOpacity="0.86" />
            </linearGradient>
            <linearGradient id="paint8_linear_402_4102" x1="16" y1="2" x2="16" y2="7" gradientUnits="userSpaceOnUse">
                <stop stopColor="#D771F1" />
                <stop offset="0.494792" stopColor="#F3E5F4" stopOpacity="0.95" />
                <stop offset="1" stopColor="#E6E7FD" stopOpacity="0.86" />
            </linearGradient>
        </defs>
    </svg>
);
export const lightIcon = (
    <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M9.49752 0.101936L9.56351 0.0937585C10.4476 -0.0158049 11.0229 0.998248 10.4752 1.70091C9.52423 2.92104 9 4.41924 9 5.99997C9 9.86596 12.134 13 16 13C17.5806 13 19.0788 12.4758 20.2989 11.5249C21.0042 10.9752 22.0212 11.5569 21.9051 12.4435C21.1926 17.8851 16.5378 22 11 22C4.92487 22 0 17.0751 0 11C0 5.48958 4.07511 0.850033 9.49752 0.101936ZM7.5488 2.89863L7.647 2.64497L7.48791 2.70976C4.2423 4.08502 2 7.3033 2 11C2 15.9705 6.02944 20 11 20C14.731 20 17.9727 17.7163 19.326 14.4255L19.354 14.353L19.1012 14.4512C18.2302 14.7708 17.3065 14.9558 16.3572 14.993L16 15C11.0294 15 7 10.9705 7 5.99997C7 4.92518 7.18927 3.87848 7.5488 2.89863Z"
            fill="url(#paint0_linear_602_2592)"
        />
        <defs>
            <linearGradient
                id="paint0_linear_602_2592"
                x1="-10.9571"
                y1="11.0428"
                x2="10.9573"
                y2="32.957"
                gradientUnits="userSpaceOnUse"
            >
                <stop stopColor="#A300FF" />
                <stop offset="0.511423" stopColor="#2C00FF" />
                <stop offset="1" stopColor="#00AFFF" />
            </linearGradient>
        </defs>
    </svg>
);
export const likeIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 40 38" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M28.0053 0.00522276C25.0652 -0.083337 22.2028 0.957153 20.0059 2.91302C17.8089 0.957153 14.9465 -0.083337 12.0064 0.00522276C8.52656 -0.00666952 5.22437 1.54096 3.00704 4.22293C1.10117 6.53477 -0.89469 10.7205 0.427219 17.544C2.53907 28.4353 18.358 37.3746 19.0259 37.7426C19.6313 38.0802 20.3684 38.0802 20.9738 37.7426C21.6478 37.3686 37.4667 28.4293 39.5725 17.544C40.8944 10.7205 38.9006 6.54477 36.9947 4.22293C34.7799 1.54321 31.4818 -0.00424021 28.0053 0.00522276ZM35.2697 16.8562C33.8172 24.3564 23.2639 31.3341 20.0119 33.3266C16.7599 31.3341 6.2066 24.3583 4.75413 16.8562C3.75397 11.6975 5.12261 8.70088 6.44835 7.09438C7.86924 5.37998 9.98284 4.39148 12.2095 4.4C14.5965 4.22098 16.9023 5.30834 18.2826 7.264C18.6211 7.8895 19.2734 8.28102 19.9846 8.28561H20.008C20.7145 8.28072 21.3644 7.89851 21.712 7.2835C23.0908 5.31233 25.4081 4.21593 27.8065 4.4C30.0359 4.38915 32.1529 5.37786 33.5755 7.09438C34.8973 8.70088 36.266 11.6975 35.2658 16.8562H35.2697Z"
            fill={color}
        />
    </svg>
);

export const commentIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 40 35" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M34 0C37.3137 0 40 2.68629 40 6V22C40 25.3137 37.3137 28 34 28H19.3333C18.9006 28 18.4795 28.1404 18.1333 28.4L11.2 33.6C9.88153 34.5889 8 33.6481 8 32V28H6C2.68629 28 0 25.3137 0 22V6C0 2.68629 2.68629 0 6 0H34ZM34 4H6C4.89543 4 4 4.89543 4 6V22C4 23.1046 4.89543 24 6 24H10C11.1046 24 12 24.8954 12 26V28L15.7333 25.2C16.7719 24.4211 18.0351 24 19.3333 24H34C35.1046 24 36 23.1046 36 22V6C36 4.89543 35.1046 4 34 4ZM12 12C13.1046 12 14 12.8954 14 14C14 15.1046 13.1046 16 12 16C10.8954 16 10 15.1046 10 14C10 12.8954 10.8954 12 12 12ZM20 12C21.1046 12 22 12.8954 22 14C22 15.1046 21.1046 16 20 16C18.8954 16 18 15.1046 18 14C18 12.8954 18.8954 12 20 12ZM28 12C29.1046 12 30 12.8954 30 14C30 15.1046 29.1046 16 28 16C26.8954 16 26 15.1046 26 14C26 12.8954 26.8954 12 28 12Z"
            fill={color}
        />
    </svg>
);
export const optionIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 40 12" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M34.68 11.3191C37.6177 11.3191 39.9991 8.93768 39.9991 6C39.9991 3.06231 37.6177 0.680847 34.68 0.680847C31.7423 0.680847 29.3608 3.06231 29.3608 6C29.3608 8.93768 31.7423 11.3191 34.68 11.3191ZM25.3183 6C25.3183 3.06231 22.9368 0.680847 19.9991 0.680847C17.0615 0.680847 14.68 3.06231 14.68 6C14.68 8.93768 17.0615 11.3191 19.9991 11.3191C22.9368 11.3191 25.3183 8.93768 25.3183 6ZM10.6374 6C10.6374 3.06231 8.25598 0.680847 5.31829 0.680847C2.38061 0.680847 -0.000854492 3.06231 -0.000854492 6C-0.000854492 8.93768 2.38061 11.3191 5.31829 11.3191C8.25598 11.3191 10.6374 8.93768 10.6374 6Z"
            fill={color}
        />
    </svg>
);
export const plusIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 35 35" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M18 0C19.5464 0 20.8 1.2536 20.8 2.8L20.799 15.2H33.2C34.7464 15.2 36 16.4536 36 18C36 19.5464 34.7464 20.8 33.2 20.8L20.799 20.799L20.8 33.2C20.8 34.7464 19.5464 36 18 36C16.4536 36 15.2 34.7464 15.2 33.2V20.799L2.8 20.8C1.2536 20.8 0 19.5464 0 18C0 16.4536 1.2536 15.2 2.8 15.2H15.2V2.8C15.2 1.2536 16.4536 0 18 0Z"
            fill={color}
        />
    </svg>
);
export const upIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 40 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M35.9406 20.5807C36.9473 21.4803 38.489 21.3891 39.3839 20.3771C40.2399 19.4091 40.1942 17.9493 39.3075 17.0369L39.1814 16.916L21.9977 1.56266C20.8596 0.545797 19.1404 0.545797 18.0023 1.56266L0.81865 16.916C-0.188136 17.8155 -0.278821 19.3651 0.6161 20.3771C1.47211 21.3451 2.91983 21.4706 3.92519 20.6924L4.05944 20.5807L19.3341 6.93317C19.7135 6.59422 20.2865 6.59422 20.6659 6.93317L35.9406 20.5807Z"
            fill={color}
        />
    </svg>
);

export const downIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 40 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M35.9406 1.41927C36.9473 0.519726 38.489 0.610879 39.3839 1.62287C40.2399 2.59086 40.1942 4.05073 39.3075 4.96309L39.1814 5.08401L21.9977 20.4373C20.8596 21.4542 19.1404 21.4542 18.0023 20.4373L0.81865 5.08401C-0.188136 4.18447 -0.278821 2.63486 0.6161 1.62287C1.47211 0.654879 2.91983 0.529393 3.92519 1.30755L4.05944 1.41927L19.3341 15.0668C19.7135 15.4058 20.2865 15.4058 20.6659 15.0668L35.9406 1.41927Z"
            fill={color}
        />
    </svg>
);

export const infoIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 36 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M18 20C31.0442 20 37.3418 26.2966 35.7609 38.0734C35.6129 39.1765 34.6713 40 33.5581 40H2.44189C1.32865 40 0.387132 39.1765 0.239052 38.0734C-1.34177 26.2966 4.95582 20 18 20ZM18 24.4444C8.29883 24.4444 4.08573 27.9326 4.46393 35.5556H31.5361C31.9143 27.9326 27.7012 24.4444 18 24.4444ZM18 0C22.91 0 26.8904 3.97969 26.8904 8.88889C26.8904 13.7981 22.91 17.7778 18 17.7778C13.09 17.7778 9.10965 13.7981 9.10965 8.88889C9.10965 3.97969 13.09 0 18 0ZM18 4.44444C15.545 4.44444 13.5548 6.43429 13.5548 8.88889C13.5548 11.3435 15.545 13.3333 18 13.3333C20.455 13.3333 22.4452 11.3435 22.4452 8.88889C22.4452 6.43429 20.455 4.44444 18 4.44444Z"
            fill={color}
        />
    </svg>
);
export const logOutIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 38 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M29.9907 30V34C29.9907 37.3137 27.3052 40 23.9925 40H5.99826C2.68551 40 0 37.3137 0 34V6C0 2.68629 2.68551 0 5.99826 0H23.9925C27.3052 0 29.9907 2.68629 29.9907 6V10C29.9907 11.1046 29.0956 12 27.9913 12C26.8871 12 25.9919 11.1046 25.9919 10V6C25.9919 4.89543 25.0967 4 23.9925 4H5.99826C4.89401 4 3.99884 4.89543 3.99884 6V34C3.99884 35.1046 4.89401 36 5.99826 36H23.9925C25.0967 36 25.9919 35.1046 25.9919 34V30C25.9919 28.8954 26.8871 28 27.9913 28C29.0956 28 29.9907 28.8954 29.9907 30ZM33.4097 14.58C32.6257 13.7958 31.3546 13.7958 30.5706 14.58C29.7865 15.3642 29.7865 16.6358 30.5706 17.42L31.1704 18H17.9942C16.89 18 15.9948 18.8954 15.9948 20C15.9948 21.1046 16.89 22 17.9942 22H31.1704L30.5706 22.58C30.192 22.9555 29.9791 23.4667 29.9791 24C29.9791 24.5333 30.192 25.0445 30.5706 25.42C30.946 25.7986 31.457 26.0116 31.9901 26.0116C32.5233 26.0116 33.0343 25.7986 33.4097 25.42L37.4086 21.42C37.7871 21.0445 38 20.5333 38 20C38 19.4667 37.7871 18.9555 37.4086 18.58L33.4097 14.58Z"
            fill={color}
        />
    </svg>
);

export const adminIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 40 35" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M26.0006 18.0014C36.1049 18.0014 41.0413 23.0079 39.8164 32.2649C39.6847 33.2595 38.8367 34.0027 37.8335 34.0027H14.1677C13.1644 34.0027 12.3164 33.2595 12.1848 32.2649C10.9598 23.0079 15.8962 18.0014 26.0006 18.0014ZM9.99933 16.0013C10.7747 16.0013 11.5065 16.0422 12.1952 16.1251C13.2919 16.2571 14.074 17.2533 13.942 18.35C13.8099 19.4467 12.8138 20.2288 11.7171 20.0968C11.1926 20.0336 10.6198 20.0016 9.99933 20.0016C5.94298 20.0016 4.14339 21.1837 4.00394 24.0019H7.99917C9.10383 24.0019 9.99933 24.8974 9.99933 26.002C9.99933 27.1067 9.10383 28.0022 7.99917 28.0022H2.11072C1.10749 28.0022 0.259463 27.259 0.127852 26.2644C-0.744222 19.6743 2.87742 16.0013 9.99933 16.0013ZM26.0006 0C30.4192 0 34.0012 3.582 34.0012 8.00063C34.0012 12.4192 30.4192 16.0013 26.0006 16.0013C21.582 16.0013 18 12.4192 18 8.00063C18 3.582 21.582 0 26.0006 0ZM9.99933 2.00016C13.3133 2.00016 15.9998 4.68666 15.9998 8.00063C15.9998 11.3146 13.3133 14.0011 9.99933 14.0011C6.68536 14.0011 3.99886 11.3146 3.99886 8.00063C3.99886 4.68666 6.68536 2.00016 9.99933 2.00016ZM9.99933 6.00047C8.89467 6.00047 7.99917 6.89597 7.99917 8.00063C7.99917 9.10528 8.89467 10.0008 9.99933 10.0008C11.104 10.0008 11.9995 9.10528 11.9995 8.00063C11.9995 6.89597 11.104 6.00047 9.99933 6.00047Z"
            fill={color}
        />
    </svg>
);

export const settingIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M20 0C22.98 0 25.5033 1.95518 26.3571 4.6527C28.8682 3.349 32.035 3.75072 34.1421 5.85786C36.2493 7.96501 36.651 11.1318 35.3473 13.6429C38.0448 14.4967 40 17.02 40 20C40 22.98 38.0448 25.5033 35.3473 26.3571C36.651 28.8682 36.2493 32.035 34.1421 34.1421C32.035 36.2493 28.8682 36.651 26.3571 35.3473C25.5033 38.0448 22.98 40 20 40C17.02 40 14.4967 38.0448 13.6429 35.3473C11.1318 36.651 7.96501 36.2493 5.85786 34.1421C3.75072 32.035 3.349 28.8682 4.6527 26.3571C1.95518 25.5033 0 22.98 0 20C0 17.02 1.95518 14.4967 4.6527 13.6429C3.349 11.1318 3.75072 7.96501 5.85786 5.85786C7.96501 3.75072 11.1318 3.349 13.6429 4.6527C14.4967 1.95518 17.02 0 20 0ZM20 4.44444C18.7727 4.44444 17.7778 5.43937 17.7778 6.66667V7.42536C17.7778 8.36717 17.1841 9.2067 16.2961 9.52056C15.9263 9.65125 15.5641 9.80151 15.2108 9.97061C14.3607 10.3774 13.3465 10.2038 12.6801 9.53744L12.1433 9.00056C11.2754 8.13273 9.86839 8.13273 9.00056 9.00056C8.13273 9.86839 8.13273 11.2754 9.00056 12.1433L9.53744 12.6801C10.2038 13.3465 10.3774 14.3607 9.97061 15.2108C9.80151 15.5641 9.65125 15.9263 9.52056 16.2961C9.2067 17.1841 8.36717 17.7778 7.42536 17.7778H6.66667C5.43937 17.7778 4.44444 18.7727 4.44444 20C4.44444 21.2273 5.43937 22.2222 6.66667 22.2222H7.42536C8.36717 22.2222 9.2067 22.8159 9.52056 23.7039C9.65125 24.0737 9.80151 24.4359 9.97061 24.7892C10.3774 25.6393 10.2038 26.6535 9.53744 27.3199L9.00056 27.8567C8.13273 28.7246 8.13273 30.1316 9.00056 30.9994C9.86839 31.8673 11.2754 31.8673 12.1433 30.9994L12.6801 30.4626C13.3465 29.7962 14.3607 29.6226 15.2108 30.0294C15.5641 30.1985 15.9263 30.3487 16.2961 30.4794C17.1841 30.7933 17.7778 31.6328 17.7778 32.5746V33.3333C17.7778 34.5606 18.7727 35.5556 20 35.5556C21.2273 35.5556 22.2222 34.5606 22.2222 33.3333V32.5746C22.2222 31.6328 22.8159 30.7933 23.7039 30.4794C24.0737 30.3487 24.4359 30.1985 24.7892 30.0294C25.6393 29.6226 26.6535 29.7962 27.3199 30.4626L27.8567 30.9994C28.7246 31.8673 30.1316 31.8673 30.9994 30.9994C31.8673 30.1316 31.8673 28.7246 30.9994 27.8567L30.4626 27.3199C29.7962 26.6535 29.6226 25.6393 30.0294 24.7892C30.1985 24.4359 30.3487 24.0737 30.4794 23.7039C30.7933 22.8159 31.6328 22.2222 32.5746 22.2222H33.3333C34.5606 22.2222 35.5556 21.2273 35.5556 20C35.5556 18.7727 34.5606 17.7778 33.3333 17.7778H32.5746C31.6328 17.7778 30.7933 17.1841 30.4794 16.2961C30.3487 15.9263 30.1985 15.5641 30.0294 15.2108C29.6226 14.3607 29.7962 13.3465 30.4626 12.6801L30.9994 12.1433C31.8673 11.2754 31.8673 9.86839 30.9994 9.00056C30.1316 8.13273 28.7246 8.13273 27.8567 9.00056L27.3199 9.53744C26.6535 10.2038 25.6393 10.3774 24.7892 9.97061C24.4359 9.80151 24.0737 9.65125 23.7039 9.52056C22.8159 9.2067 22.2222 8.36717 22.2222 7.42536V6.66667C22.2222 5.43937 21.2273 4.44444 20 4.44444ZM20 15.5556C22.4546 15.5556 24.4444 17.5454 24.4444 20C24.4444 22.4546 22.4546 24.4444 20 24.4444C17.5454 24.4444 15.5556 22.4546 15.5556 20C15.5556 17.5454 17.5454 15.5556 20 15.5556Z"
            fill={color}
        />
    </svg>
);
export const verifyIcon = (color, width, height) => (
    <svg width={width} height={height} viewBox="0 0 35 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
            d="M23.2129 14.7675L16.1954 21.7745L12.7871 18.3713C12.1496 17.7346 11.1159 17.7346 10.4782 18.3713C9.84059 19.0079 9.84059 20.04 10.4782 20.6767L15.041 25.2328C15.6785 25.8693 16.7122 25.8695 17.3499 25.2328L25.5217 17.073C26.1594 16.4364 26.1594 15.4042 25.5218 14.7675C24.8842 14.1308 23.8505 14.1309 23.2129 14.7675Z"
            fill={color}
        />
        <path
            fillRule="evenodd"
            clipRsule="evenodd"
            d="M18.5158 0.259899C17.6801 -0.0864374 16.7405 -0.0868544 15.904 0.259976L2.10224 5.98185C0.831029 6.50897 0 7.74954 0 9.12807V16.0837C0 26.4757 6.29088 35.8222 15.923 39.7478C16.748 40.0841 17.6721 40.0839 18.497 39.7478C28.129 35.8222 34.42 26.4757 34.42 16.0837V9.12807C34.42 7.74967 33.5891 6.50896 32.3176 5.9818L18.5158 0.259899ZM30.42 9.52521L17.21 4.04868L4 9.52521V16.0837C4 24.7665 9.20713 32.5862 17.21 35.9515C25.2127 32.5863 30.42 24.7666 30.42 16.0837V9.52521Z"
            fill={color}
        />
    </svg>
);

export const facbookIcon = (color, width, height) => (
    <svg
        fill={color}
        height={height}
        width={width}
        version="1.1"
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="-337 273 123.5 256"
        xmlSpace="preserve"
    >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
        <g id="SVGRepo_iconCarrier">
            {' '}
            <path d="M-260.9,327.8c0-10.3,9.2-14,19.5-14c10.3,0,21.3,3.2,21.3,3.2l6.6-39.2c0,0-14-4.8-47.4-4.8c-20.5,0-32.4,7.8-41.1,19.3 c-8.2,10.9-8.5,28.4-8.5,39.7v25.7H-337V396h26.5v133h49.6V396h39.3l2.9-38.3h-42.2V327.8z"></path>{' '}
        </g>
    </svg>
);
export const githubIcon = (color, width, height) => (
    <svg
        fill={color}
        height={height}
        width={width}
        version="1.1"
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="-267 288.9 264.5 225.1"
        xmlSpace="preserve"
    >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
        <g id="SVGRepo_iconCarrier">
            {' '}
            <g>
                {' '}
                <path d="M-21.8,354.4c-0.8-0.9-1.3-2.3-1.2-3.5c0.9-20.1-1.8-39.6-8.6-58.6c-0.9-2.4-2-3.3-4.5-2.6c-5.6,1.7-11.4,3-16.9,5 c-15.2,5.5-29.1,13.5-42.6,22.2c-1.4,0.9-3.5,1.5-5.2,1.2c-22.1-3.9-44.3-3.7-66.5-0.3c-2.2,0.3-5-0.2-6.8-1.3 c-15.7-10.2-31.7-20.2-50-25c-15.1-4-12-4.8-16.3,8.9c-5.1,16.4-7.1,33.4-6,50.6c0.1,1.2-0.8,2.7-1.6,3.8 c-6.6,7.9-11.7,16.6-14.8,26.4c-6,19-4.8,38.2-1,57.3c7.5,37.5,32.8,63.8,70.2,70.3c19.3,3.4,39.2,3.7,57.3,5.2 c20.2-1.5,38.9-1.6,57.1-4.5c31.8-5.1,55.8-22,67.8-52.7c4.2-10.7,6.5-22.4,7.9-33.8C-0.3,397.9-4.6,374.3-21.8,354.4z M-39.5,458.8c-3.3,15.6-12.4,26.3-27.6,31.8c-14.7,5.4-29.9,7.6-45.4,8.6c-7.5,0.5-15,0.1-22.5,0.1c-20.2,0.4-40.4-0.4-59.9-6.2 c-24.5-7.3-35.5-21.9-36.2-47.5c-0.3-9.1,0.7-17.9,5-26.1c8.5-16.5,23.2-22.3,40.6-22.6c9.5-0.1,19.1,0.7,28.7,1.6 c20.1,1.8,40-0.1,60-1.2c8.8-0.5,17.8-0.7,26.4,0.8c18.5,3.2,32.5,21.6,32.8,42.3C-37.6,446.5-38.3,452.8-39.5,458.8z"></path>{' '}
                <path d="M-191.5,424.5c-5.5,5.7-7.4,12.9-7.7,19.3c0,9.6,2.8,17.3,7.7,22.3c6.8,6.9,16.1,6.7,22.6-0.3c9.8-10.6,9.7-30.5,0-41 C-175.4,417.7-184.9,417.6-191.5,424.5z"></path>{' '}
                <path d="M-99.4,423.7c-11.2,10.3-11.2,33,0,43.2c6.5,5.9,15.3,5.6,21.4-0.7c5.6-5.8,7.6-13.1,7.8-21c-0.2-7.9-2.2-15.1-7.9-20.9 C-84.2,418-93,417.8-99.4,423.7z"></path>{' '}
            </g>{' '}
        </g>
    </svg>
);
export const instagramIcon = (color, width, height) => (
    <svg
        fill={color}
        height={height}
        width={width}
        version="1.1"
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="-271 273 256 256"
        xmlSpace="preserve"
    >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g>
        <g id="SVGRepo_iconCarrier">
            {' '}
            <path d="M-64.5,273h-157c-27.3,0-49.5,22.2-49.5,49.5v52.3v104.8c0,27.3,22.2,49.5,49.5,49.5h157c27.3,0,49.5-22.2,49.5-49.5V374.7 v-52.3C-15.1,295.2-37.3,273-64.5,273z M-50.3,302.5h5.7v5.6v37.8l-43.3,0.1l-0.1-43.4L-50.3,302.5z M-179.6,374.7 c8.2-11.3,21.5-18.8,36.5-18.8s28.3,7.4,36.5,18.8c5.4,7.4,8.5,16.5,8.5,26.3c0,24.8-20.2,45.1-45.1,45.1s-44.9-20.3-44.9-45.1 C-188.1,391.2-184.9,382.1-179.6,374.7z M-40,479.5C-40,493-51,504-64.5,504h-157c-13.5,0-24.5-11-24.5-24.5V374.7h38.2 c-3.3,8.1-5.2,17-5.2,26.3c0,38.6,31.4,70,70,70c38.6,0,70-31.4,70-70c0-9.3-1.9-18.2-5.2-26.3H-40V479.5z"></path>{' '}
        </g>
    </svg>
);
